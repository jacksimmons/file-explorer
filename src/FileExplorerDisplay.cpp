...
FileExplorerDisplay::FileExplorerDisplay()
{
...
bool FileExplorerDisplay::Draw(bool update)
{
	if (!m_active)
		return false;
	...
	ImGui::TableNextColumn(); bool goBack = ImGui::Button("../");
	ImGui::TableNextColumn();
	...
	for (auto &file : m_currentFiles)
	{
		...
		// Name col
		ImGui::TableNextColumn();
		ImGui::PushStyleColor(ImGuiCol_Text, GetTextColourFromAccess(access));
		selected = ImGui::Selectable((name + "##FILE").c_str(), selected);
		ImGui::PopStyleColor();
		
		// File size col
		ImGui::TableNextColumn();
		ImGui::Text((std::to_string(fileSizeDenomValue) + " " + fileSizeDenom).c_str());

		// If the file was clicked on, mark it as selected, unless access is denied
		if (selected && name != ACCESS_DENIED)
		{
			m_selectedFile = name;
		}
	}
...
void FileExplorerDisplay::UpdateFiles()
{
...
fs::path FileExplorerDisplay::currentPath()
{
	return m_currentPath;
}

std::string FileExplorerDisplay::selectedFile()
{
